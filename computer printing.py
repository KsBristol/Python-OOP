...
total_mem_slots = 4 - общее число слотов памяти (атрибут прописывается с этим значением
и не меняется); mem_slots - список из объектов класса Memory (максимум total_mem_slots = 4 штук
по максимальному числу слотов памяти).

Класс MotherBoard должен иметь метод get_config(self) для возвращения текущей конфигурации
компонентов на материнской плате в виде следующего списка из четырех строк:

['Материнская плата: <наименование>',
'Центральный процессор: <наименование>, <тактовая частота>',
'Слотов памяти: <общее число слотов памяти>',
'Память: <наименование_1> - <объем_1>; <наименование_2> - <объем_2>; ...; <наименование_N> -
<объем_N>']

Создайте объект mb класса MotherBoard с одним CPU (объект класса CPU) и двумя слотами памяти
(объекты класса Memory).
...
class MotherBoard:
    ...
    def __init__(self, name, cpu, *mems):
        """
        :param name: наименование
        :param cpu: ссылка на объект класса CPU
        """
        self.name = name
        self.cpu = cpu
    ...
    def get_config(self):
        ...
        <объем_N>']
        """
        self.itog = [f"Материнская плата: {self.name}",
...
mb = MotherBoard("Asus", CPU("Intel", 2000), Memory("Kingston", 3000), Memory("Kingston", 1000))

print(mb.get_config())

# ['Материнская плата: Asus',
# 'Центральный процессор: Intel, 2000',
# 'Слотов памяти: 4', '
# Память: Kingston - 3000; Kingston - 1000']
